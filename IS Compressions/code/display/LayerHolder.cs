...
internal class LayerHolder
{
    ...
    private void ResetDrawAtLayer(int index)
    {
        ...
        var width = (int)Math.Ceiling((float)layers[index].GetWidth());
        var height = (int)Math.Ceiling((float)layers[index].GetHeight());


        for(var sWidth = Display.tileSize * (startX / Display.tileSize); sWidth < startX + width; sWidth += Display.tileSize)
        {
            for (var sHeight = Display.tileSize * (startY / Display.tileSize); sHeight < startY + height; sHeight += Display.tileSize)
            {
    ...
    public LayerHolder(Layer backgroundLayer, RenderWindow screen)
    {
        ...
        this.screen = screen;
        this.vertices = new List<Vertex>(Display.tileSize * Display.tileSize * 4);
    ...
    {
        layers[index].Render(ref screenColorCache, drawCoords);
    }
    public void RenderEntire()
    {
        foreach (Layer layer in layers)
        {
            ...
            {
                layer.Render(ref screenColorCache, drawCoords);
            }
        }
        ...
        {
            for(var x = coord.X; x < coord.X + Display.tileSize; x++)
            {
                for(var y = coord.Y; y < coord.Y + Display.tileSize; y++)
                {
                    /*
